...
// Test the SuperFarm NFT-only ERC-1155 implementation.
describe('Fee1155NFT', function () {
	const startingUri = 'starting-uri';
	let alice, bob;
	let FeeOwner, MockProxyRegistry, Fee1155NFT;
	before(async () => {
		...
		Fee1155NFT = await ethers.getContractFactory('Fee1155NFT');
	});
	...
	beforeEach(async () => {
		...
		itemOne = await Fee1155NFT.connect(alice.signer).deploy(startingUri, itemFeeOwner.address, proxyRegistry.address);
	...
	it('should allow owner to create items', async () => {
		let createItemGroupTransaction = await itemOne.connect(alice.signer).createNFT(alice.address, 200, []);
	...
	it('should prevent non-owners from creating items', async () => {
		await expect(
			itemOne.connect(bob.signer).createNFT(bob.address, 200, [])
		).to.be.revertedWith('Ownable: caller is not the owner');
	});
});
